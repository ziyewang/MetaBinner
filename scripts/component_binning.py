    ...
    for k in range(candK, maxK, stepK):
        if k < len(len_weight):
            if dataset_scale=='huge':
                kmeans = MiniBatchKMeans(n_clusters=k, random_state=7, n_init=30, init_size=min(len(len_weight),max(3072,3*k)))
            else:
...
def my_kmeans(X_mat, namelist, bin_number, bacar_marker_seed_num, length_weight, output, contig_length_threshold,
    ...
    if not (os.path.exists(output_temp)):
        if dataset_scale == 'huge':
            km = MiniBatchKMeans(n_clusters=bin_number, random_state=7, n_init=30, init_size=min(len(length_weight),max(3072,3*bin_number)))
        else:
